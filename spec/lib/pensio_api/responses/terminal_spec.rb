...
describe PensioAPI::Responses::Terminal do
  ...
  describe 'object mapping' do
    ...
    describe '.map_terminals' do
      context 'with one terminal' do
        before :each do
          stub_pensio_response('/merchant/API/getTerminals', 'get_terminals_single', http_verb: 'GET')
        end
  ...
  context 'with no terminals' do
    before :each do
      stub_pensio_response('/merchant/API/getTerminals', 'get_terminals_none', http_verb: 'GET')
    end
